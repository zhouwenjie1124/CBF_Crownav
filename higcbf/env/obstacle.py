...
class Circle(NamedTuple):
    ...
    def inside(self, point: Pos2d, r: Radius = 0.) -> BoolScalar:
        ...
        return dist < (self.radius + r)

    def raytracing(self, start: Pos2d, end: Pos2d) -> Array:
        # Ray from start to end: start + alpha * (end - start)
        d = end - start
        f = start - self.center
        a = jnp.dot(d, d)
        b = 2.0 * jnp.dot(f, d)
        ...
        denom = 2.0 * jnp.maximum(a, 1e-9)
        t1 = (-b - sqrt_disc) / denom
        t2 = (-b + sqrt_disc) / denom
        t1 = jnp.where(valid & (t1 >= 0.0) & (t1 <= 1.0), t1, 1e6)
        t2 = jnp.where(valid & (t2 >= 0.0) & (t2 <= 1.0), t2, 1e6)
...
class MixedObstacle(NamedTuple):
    ...
    def inside(self, point: Pos2d, r: Radius = 0.) -> BoolScalar:
        ...
        is_out_corner = jnp.logical_and(rel_xx > 0, rel_yy > 0)
        is_in_circle = jnp.sqrt(rel_xx ** 2 + rel_yy ** 2) < r
        rect_in = jnp.logical_or(jnp.logical_or(is_in_down, is_in_up), jnp.logical_and(is_out_corner, is_in_circle))

        # circle inside
        diff = point - self.center
        dist = jnp.sqrt(jnp.sum(diff ** 2))
        circle_in = dist < (self.radius + r)

        return jnp.where(self.type == CIRCLE, circle_in, rect_in)
    ...
    def raytracing(self, start: Pos2d, end: Pos2d) -> Array:
        ...
        t1 = jnp.where(valid & (t1 >= 0.0) & (t1 <= 1.0), t1, 1e6)
        t2 = jnp.where(valid & (t2 >= 0.0) & (t2 <= 1.0), t2, 1e6)
        alpha_circle = jnp.minimum(t1, t2)

        return jnp.where(self.type == CIRCLE, alpha_circle, alpha_rect)

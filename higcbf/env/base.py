...
class MultiAgentEnv(ABC):
    ...
    def collision_mask(self, graph: GraphsTuple) -> Array:
        pass
    # rollout_fn：用 lax.scan 纯 JAX 循环，一次性编译/执行，返回 JAX array，适合训练/并行（可 jit/vmap）。
    def rollout_fn(self, policy: Callable, rollout_length: int = None) -> Callable[[PRNGKey], RolloutResult]:
        ...
        def body(carry, _):
            ...
            return (graph_new, done_next), (graph_new, action, reward, cost, done, info)

        def fn(key: PRNGKey) -> RolloutResult:
            graph0 = self.reset(key)
            init_carry = (graph0, jnp.array(False))
            (graph_final, _), (T_graph, T_action, T_reward, T_cost, T_done, T_info) = lax.scan(
    ...
    #rollout_fn_jitstep 只把单步 body 做 jit，外层用 Python for 循环 + tqdm，每步都能拿到结果并转成 NumPy，适合可视化/调试/保存视频。
    def rollout_fn_jitstep(
        ...
        def fn(key: PRNGKey) -> [RolloutResult, Array, Array]:
            ...
            for kk in tqdm.trange(rollout_length, ncols=80):
                (graph, done), output = jit_body((graph, done), None)

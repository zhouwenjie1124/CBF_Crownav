...
def rollout(
    ...
    def body(carry, key_):
        ...
        # If already done, keep state and zero out rewards/costs
        ...
        next_graph = jtu.tree_map(lambda new, old: jnp.where(done_prev, old, new), next_graph, graph)

        return (next_graph, done_next), (graph, action, reward, cost, done, log_pi, next_graph)

    keys = jax.random.split(key, env.max_episode_steps)
    init_carry = (init_graph, jnp.array(False))
    (final_graph, _), (graphs, actions, rewards, costs, dones, log_pis, next_graphs) = \
        jax.lax.scan(body, init_carry, keys, length=env.max_episode_steps)
...
def rollout_rnn(
    ...
    """
    Recurrent rollout.
    actor: [graph, key, carry, done_prev] -> [action, log_pi, new_carry]
    """
    key_x0, key = jax.random.split(key)
    init_graph = env.reset(key_x0)

    def body(carry_pack, key_):
        graph, done_prev, actor_carry = carry_pack
        action, log_pi, next_actor_carry = actor(graph, key_, actor_carry, done_prev)
        next_graph, reward, cost, done, info = env.step(graph, action)
        done_next = jnp.logical_or(done_prev, done)
        ...
        next_actor_carry = jtu.tree_map(
            lambda new, old: jnp.where(done_prev[..., None], old, new), next_actor_carry, actor_carry
        )

        return (next_graph, done_next, next_actor_carry), (graph, action, reward, cost, done, log_pi, next_graph)

    keys = jax.random.split(key, env.max_episode_steps)
    init_carry_pack = (init_graph, jnp.array(False), init_carry)
    (_, _, _), (graphs, actions, rewards, costs, dones, log_pis, next_graphs) = \
        jax.lax.scan(body, init_carry_pack, keys, length=env.max_episode_steps)
    data = Rollout(graphs, actions, rewards, costs, dones, log_pis, next_graphs)
    return data
